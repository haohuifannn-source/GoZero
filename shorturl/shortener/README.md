...
本项目用的是布隆过滤器
***生产建议***：如果布隆过滤器报错（比如 Redis 挂了），为了业务可用性，通常选择“放行”而不是报错；同时一般都是不存在就肯定返回，但是存在不能返回，因为布隆过滤器会存在误报，得去缓存或者数据库确认

1. 首先在svc文件去定义和初始化布隆过滤器
2. 然后在conertLogic里面加载到布隆过滤器
3. 最后在查询的地方加入布隆过滤器的防御

因为布隆过滤器不支持删除，因此拓展一个布谷鸟过滤器（本项目很适合，是因为不会生成一样的短链接）
